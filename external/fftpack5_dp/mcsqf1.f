...
C
C   $Id: mcsqf1.f,v 1.2 2006/11/21 01:10:17 haley Exp $
C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC

      SUBROUTINE DMCSQF1(LOT,JUMP,N,INC,X,WSAVE,WORK,IER)
      DOUBLE PRECISION X
  ...
  303 CONTINUE
      ...
      LNWK = LOT*N
C
      CALL DRFFTMF(LOT,JUMP,N,INC,X,LENX,WSAVE(N+1),LNSV,WORK,LNWK,IER1)
      IF (IER1.NE.0) THEN
          IER = 20
          CALL DXERFFT('MCSQF1',-5)
          GO TO 400
      END IF
C
      DO 103 I = 3,N,2
          DO 203 M = 1,LJ,JUMP
              XIM1 = .5D0* (X(M,I-1)+X(M,I))

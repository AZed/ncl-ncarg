C
C       $Id: curvs1.f,v 1.4 2008/07/27 03:10:11 haley Exp $
C
C                Copyright (C)  2000
...
C
C this subroutine references package module CURVS.
C
      REAL X(N),Y(N),D(N),S,EPS,PARAM(N),XS(N),YS(N),XSP(N),YSP(N),
     +     SIGMA,TEMP(N,20)
C
...
C
      CALL CURVS(N,PARAM,X,D,ISW,S,EPS,XS,XSP,SIGMA,TEMP(1, 2),IERR)
      CALL CURVS(N,PARAM,Y,D,ISW,S,EPS,YS,YSP,SIGMA,TEMP(1,11),IERR)
C
      RETURN
      END

C
C $Id: curvpsdp.f,v 1.3 2008/07/27 03:10:10 haley Exp $
C
C                Copyright (C)  2000
...
C
      SUBROUTINE CURVPSDP(N,X,Y,P,D,ISW,S,EPS,YS,YSP,SIGMA,TEMP,IERR)
c
      INTEGER N,ISW,IERR
      DOUBLE PRECISION X(N),Y(N),P,D(N),S,EPS,YS(N),YSP(N),SIGMA,
     +                 TEMP(N,11)
c
...
c curve it is necessary to call the function curvp2dp. the
...
c
     ...
     +            TEMP(1,8),TEMP(1,9),TEMP(1,10),TEMP(1,11),IERR)
      RETURN
      END

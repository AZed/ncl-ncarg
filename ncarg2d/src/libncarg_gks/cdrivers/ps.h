/*
 *	$Id: ps.h,v 1.7 2008/07/23 17:29:43 haley Exp $
 */
/************************************************************************
*                                                                       *
*                Copyright (C)  2000                                    *
 ...
 *
 *      Description:	PostScript device driver header file
 */
#ifndef	_ps_driver_
#define	_ps_driver_
...
#define PS_COLR_IND_DEFAULT             1

#define NUM_PS_FONTS			13
#define DEFAULT                         1
...
#define PS_TIMES_ROMAN			0
#define PS_TIMES_BOLD			1
#define PS_TIMES_ITALIC			2
#define PS_TIMES_BOLDITALIC		3
#define PS_HELVETICA			4
#define PS_HELVETICA_BOLD		5
#define PS_HELVETICA_OBLIQUE		6
#define PS_HELVETICA_BOLDOBLIQUE	7
#define PS_COURIER			8
#define PS_COURIER_BOLD			9
#define PS_COURIER_OBLIQUE		10
#define PS_COURIER_BOLDOBLIQUE		11
#define PS_SYMBOL			12
...
typedef	struct	PSPoint_	{
	float	x,y;
	} PSPoint;

typedef	struct	PSColor_	{
	float	r,g,b;
	} PSColor;

typedef struct 	PSDeviceSpace_	{
	int	llx,
		lly,
		urx,
		ury,
...
typedef struct 	PSCharInfo_ {
	...
	int 	char_width;     /* The character width in a normalized PS */
				/*   font scaled by 1000 */
	int     font_height;    /* Height of standard capital in current font */
	int	outline;	/* Flags outlines (TRUE) or filled (FALSE)  */
	} PSCharInfo;

typedef struct 	PSTextent_ {	/* Text extent in font and viewport space */
	int	left;
	int	right;
	int	bottom;
	int	top;
...
typedef	struct	PSClipRect_ {	/* Current clipping rectangle in PS coords. */
...
typedef struct	PSattribute_ {
	...
	int	fill_colr_ind;
	int	clip_ind;
	int	ps_colr_ind;        /*  Index of current PS color being used */
	int	null_clip_rect;     /*  Flags a null clipping rectangle */
	} PSattribute;

typedef enum {MONO, COLOR} ps_color;
typedef enum {RPS, EPSF, EPSI} ps_file_type;
typedef enum {PORTRAIT, LANDSCAPE} ps_orientation;
typedef enum {DO_NOTHING, MOVETO, RMOVETO, LINETO, NEW_LINE, 

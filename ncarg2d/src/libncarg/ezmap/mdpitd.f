C
C $Id: mdpitd.f,v 1.9 2008/09/18 00:42:17 kennison Exp $
C
C                Copyright (C)  2000
...
C
      SUBROUTINE MDPITD (XLAT,XLON,IFST)
C
...
C
        IF (ICFELL('MDPITD - UNCLEARED PRIOR ERROR',1).NE.0) RETURN
C
C If EZMAP needs initialization, do it.
C
        IF (INTF) THEN
          CALL MDPINT
          IF (ICFELL('MDPITD',2).NE.0) RETURN
        END IF
C
...
C
        ...
        IF (ICFELL('MDPITD',3).NE.0) RETURN
        PNEW=P
C
...
C
     ...
     +                   RLTE,RLNE,PAPE,UCOE,VCOE)
            IF (ICFELL('MDPITD',4).NE.0) RETURN
            CALL MDPVPD (UOLD,VOLD,UCOE,VCOE)
            IF (ICFELL('MDPITD',5).NE.0) RETURN
          END IF
          GO TO 103
        END IF
C
...
C
        IF (IFST.EQ.0) THEN
          IF (IDTL.EQ.0) THEN
            CALL DPFRST (REAL(UNEW),REAL(VNEW))
            IF (ICFELL('MDPITD',6).NE.0) RETURN
          ELSE
            DATL=0.D0
          END IF
          GO TO 103
...
C
     ...
     +                 RLTO,RLNO,POLD,UOLD,VOLD)
          IF (ICFELL('MDPITD',7).NE.0) RETURN
          IVSO=1
          IF (IDTL.EQ.0) THEN
            CALL DPFRST (REAL(UOLD),REAL(VOLD))
            IF (ICFELL('MDPITD',8).NE.0) RETURN
          ELSE
            DATL=0.D0
          END IF
        END IF
...
C
     ...
     +               RLAT,RLON,PNEW,UNEW,VNEW,
     +               RLTE,RLNE,PAPE,UCOE,VCOE)
        IF (ICFELL('MDPITD',9).NE.0) RETURN
        CALL MDPVPD (UOLD,VOLD,UCOE,VCOE)
        IF (ICFELL('MDPITD',10).NE.0) RETURN
C
        CALL MDITVE (RLAT,RLON,PNEW,UNEW,VNEW,
     +               RLTO,RLNO,POLD,UOLD,VOLD,
     +               RLTO,RLNO,POLD,UOLD,VOLD)
        IF (ICFELL('MDPITD',11).NE.0) RETURN
C
...
C
        ...
        IF (IDTL.EQ.0) THEN
          CALL DPFRST (REAL(UOLD),REAL(VOLD))
          IF (ICFELL('MDPITD',12).NE.0) RETURN
        ELSE
          DATL=0.D0
        END IF
C
...
C
  102   IF ((IOPI.LE.NOPI.OR.IFST.LT.2).AND.
     +      ((UNEW-UOLD)**2+(VNEW-VOLD)**2)*DSSQ.LE.DPSQ)GO TO 104
        CALL MDPVPD (UOLD,VOLD,UNEW,VNEW)
        IF (ICFELL('MDPITD',13).NE.0) RETURN
C

C
C $Id: pjinsp.f,v 1.7 2008/07/27 00:17:12 haley Exp $
C
C                Copyright (C)  2000
...
C
      SUBROUTINE PJINSP (ISYS,ZONE,ISPH,DATA)
C
      IMPLICIT REAL (A-Z)
      REAL SECS(5)
      INTEGER ITEMP
...
C
      ...
      COMMON /SPHRSP/ AZZ
      COMMON /NORMSP/ Q,T,U,W,ES22,P22,SA,CA,XJ
      COMMON /PC03SP/ A03,LON003,X003,Y003,C,E03,ES03,NS03,RH003
      COMMON /PC04SP/ A04,LON004,X004,Y004,E04,F04,NS04,RH004
      COMMON /PC05SP/ A05,LON005,X005,Y005,E05,M1
      COMMON /PC06SP/ A06,LON006,X006,Y006,E06,E4,FAC,MCS,TCS,IND06
      COMMON /PC07SP/ A07,LON007,X007,Y007,E07,E007,E107,E207,E307,ES07,
     .                ML007
      COMMON /PC08SP/ A08,LON008,X008,Y008,E008,E108,E208,E308,GL,NS08,
     .                RH008
      COMMON /PC09SP/ A09,LON009,X009,Y009,ES09,ESP,E009,E109,E209,E309,
     .                KS009,LAT009,ML009,IND09
      COMMON /PC10SP/ A10,LON010,X010,Y010,COSP10,LAT010,SINP10
      COMMON /PC11SP/ A11,LON011,X011,Y011,COSP11,LAT011,SINP11
      COMMON /PC12SP/ A12,LON012,X012,Y012,COSP12,LAT012,SINP12
      COMMON /PC13SP/ A13,LON013,X013,Y013,COSP13,LAT013,SINP13
      COMMON /PC14SP/ A14,LON014,X014,Y014,COSP14,LAT014,SINP14
      COMMON /PC15SP/ A15,LON015,X015,Y015,COSP15,LAT015,P,SINP15
      COMMON /PC16SP/ A16,LON016,X016,Y016
      COMMON /PC17SP/ A17,LON017,X017,Y017,LAT1
      COMMON /PC18SP/ A18,LON018,X018,Y018
      COMMON /PC19SP/ A19,LON019,X019,Y019
      COMMON /PC20SP/ LON020,X020,Y020,AL,BL,COSALF,COSGAM,E20,EL,
     +                SINALF,SINGAM,U0
      COMMON /PC21SP/ A21,LON021,X021,Y021,PR,XLR
      COMMON /PC22SP/ A22,X022,Y022,A2,A4,B,C1,C3,LAND,PATH
...
C
      IPRF=0
      IERR=0
      IF (ISYS.NE.2) CALL SPHDSP (ISPH,DATA)
C
...
C
  ...
  140       FORMAT (/' ERROR PJ01SP'/
     .               ' ILLEGAL ZONE NO. : ',I10)
         ...
         DATA(3) = 0.9996E0
         DATA(4) = ZERO
         DATA(5) = REAL(6*ZONE-183)
         DATA(6) = ZERO
         DATA(7) = 500000.0E0
         DATA(8) = ZERO
         IF (KEEPZN .LT. 0) DATA(8) = 10000000.0E0
         ITEMP = IPPARM
         IPPARM = 1
         AZ = DATA(1)
         EZ = DATA(2)
...
C
  ...
  220          CONTINUE
         ...
         IF (ISPH.EQ.0) THEN
            PNAME=NM27(IND02)
            ID=ID27(IND02)
            TABLE(1)=REAL(PA27(1,IND02))
            TABLE(2)=REAL(PA27(2,IND02))
            TABLE(3)=REAL(PA27(3,IND02))
            TABLE(4)=REAL(PA27(4,IND02))
            TABLE(5)=REAL(PA27(5,IND02))
            TABLE(6)=REAL(PA27(6,IND02))
            TABLE(7)=REAL(PA27(7,IND02))
            TABLE(8)=REAL(PA27(8,IND02))
            TABLE(9)=REAL(PA27(9,IND02))
         ELSE IF (ISPH.EQ.8) THEN
            PNAME=NM83(IND02)
            ID=ID83(IND02)
            TABLE(1)=REAL(PA83(1,IND02))
            TABLE(2)=REAL(PA83(2,IND02))
            TABLE(3)=REAL(PA83(3,IND02))
            TABLE(4)=REAL(PA83(4,IND02))
            TABLE(5)=REAL(PA83(5,IND02))
            TABLE(6)=REAL(PA83(6,IND02))
            TABLE(7)=REAL(PA83(7,IND02))
            TABLE(8)=REAL(PA83(8,IND02))
            TABLE(9)=REAL(PA83(9,IND02))
         END IF
         IF (ID .LE. 0) THEN
            IF (IPEMSG .EQ. 0) WRITE (IPELUN,240) ZONE, ISPH
  240       FORMAT(/' ERROR PJ02SP'/
...
C
         IF (ID.EQ.1) THEN
            DATA(3) = TABLE(4)
            DATA(5) = PAKRSP(PAKCSP(TABLE(3)))/.017453292519943E0
            DATA(6) = PAKRSP(PAKCSP(TABLE(7)))/.017453292519943E0
            DATA(7) = TABLE(8)
            DATA(8) = TABLE(9)
            GO TO 900
         END IF
...
C
         IF (ID.EQ.2) THEN
            DATA(3) = PAKRSP(PAKCSP(TABLE(6)))/.017453292519943E0
            DATA(4) = PAKRSP(PAKCSP(TABLE(5)))/.017453292519943E0
            DATA(5) = PAKRSP(PAKCSP(TABLE(3)))/.017453292519943E0
            DATA(6) = PAKRSP(PAKCSP(TABLE(7)))/.017453292519943E0
            DATA(7) = TABLE(8)
            DATA(8) = TABLE(9)
...
C
         IF (ID.EQ.3) THEN
            DATA(5) = PAKRSP(PAKCSP(TABLE(3)))/.017453292519943E0
            DATA(6) = PAKRSP(PAKCSP(TABLE(4)))/.017453292519943E0
            DATA(7) = TABLE(5)
            DATA(8) = TABLE(6)
            GO TO 700
         END IF
...
C
         IF (ID.EQ.4) THEN
            DATA(3) = TABLE(4)
            DATA(4) = PAKRSP(PAKCSP(TABLE(6)))/.017453292519943E0
            DATA(5) = PAKRSP(PAKCSP(TABLE(3)))/.017453292519943E0
            DATA(6) = PAKRSP(PAKCSP(TABLE(7)))/.017453292519943E0
            DATA(7) = TABLE(8)
            DATA(8) = TABLE(9)
...
C
  300    A03 = AZ
         E03 = EZ
         ES03 = ESZ
         LAT1 = .017453292519943E0 * DATA(3)
         LAT2 = .017453292519943E0 * DATA(4)
         IF (ABS(LAT1+LAT2) .LT. EPSLN) THEN
            IF (IPEMSG .EQ. 0) WRITE (IPELUN,340)
  340       FORMAT (/' ERROR PJ03SP'/
     .               ' EQUAL LATITUDES FOR ST. PARALLELS ON OPPOSITE',
     .               ' SIDES OF EQUATOR')
            IERR = 031
            RETURN
         END IF
         LON003 = .017453292519943E0 * DATA(5)
         LAT003 = .017453292519943E0 * DATA(6)
         X003 = DATA(7)
         Y003 = DATA(8)
         SINP03 = SIN (LAT1)
         CON = SINP03
         COSP03 = COS (LAT1)
         MS1 = MSFNSP (E03,SINP03,COSP03)
         QS1 = QSFNSP (E03,SINP03,COSP03)
         SINP03 = SIN (LAT2)
         COSP03 = COS (LAT2)
         MS2 = MSFNSP (E03,SINP03,COSP03)
         QS2 = QSFNSP (E03,SINP03,COSP03)
         SINP03 = SIN (LAT003)
         COSP03 = COS (LAT003)
         QS0 = QSFNSP (E03,SINP03,COSP03)
...
C
         CALL RADDSP (LAT1,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT2,SGNA(2),DEGS(2),MINS(2),SECS(2))
         CALL RADDSP (LON003,SGNA(3),DEGS(3),MINS(3),SECS(3))
         CALL RADDSP (LAT003,SGNA(4),DEGS(4),MINS(4),SECS(4))
...
C
  ...
  440       FORMAT (/' ERROR PJ04SP'/
     .               ' EQUAL LATITUDES FOR ST. PARALLELS ON OPPOSITE',
     .               ' SIDES OF EQUATOR')
            IERR = 041
            RETURN
         END IF
         LON004 = .017453292519943E0 * DATA(5)
         LAT004 = .017453292519943E0 * DATA(6)
         X004 = DATA(7)
         Y004 = DATA(8)
         SINP04 = SIN (LAT1)
         CON = SINP04
         COSP04 = COS (LAT1)
         MS1 = MSFNSP (E04,SINP04,COSP04)
         TS1 = TSFNSP (E04,LAT1,SINP04)
         SINP04 = SIN (LAT2)
         COSP04 = COS (LAT2)
         MS2 = MSFNSP (E04,SINP04,COSP04)
         TS2 = TSFNSP (E04,LAT2,SINP04)
         SINP04 = SIN (LAT004)
         TS0 = TSFNSP (E04,LAT004,SINP04)
         IF (ABS(LAT1-LAT2) .GE. EPSLN) THEN
...
C
         CALL RADDSP (LAT1,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT2,SGNA(2),DEGS(2),MINS(2),SECS(2))
         CALL RADDSP (LON004,SGNA(3),DEGS(3),MINS(3),SECS(3))
         CALL RADDSP (LAT004,SGNA(4),DEGS(4),MINS(4),SECS(4))
...
C
  500    A05 = AZ
         E05 = EZ
         ES = ESZ
         LON005 = .017453292519943E0 * DATA(5)
         LAT1 = .017453292519943E0 * DATA(6)
...
C
         CALL RADDSP (LAT1,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LON005,SGNA(2),DEGS(2),MINS(2),SECS(2))
...
C
  600    A06 = AZ
         ...
         ES = ESZ
         E4 = E4Z
         LON006 = .017453292519943E0 * DATA(5)
         SAVE = DATA(6)
         LATC = .017453292519943E0 * SAVE
         X006 = DATA(7)
         Y006 = DATA(8)
         FAC = ONE
         ...
         IF (ABS(SAVE) .NE. NINTYD) THEN
            ...
            SINPHI = SIN (CON1)
            COSPHI = COS (CON1)
            MCS = MSFNSP (E06,SINPHI,COSPHI)
            TCS = TSFNSP (E06,CON1,SINPHI)
         END IF
C
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON006,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LATC,SGNA(2),DEGS(2),MINS(2),SECS(2))
...
C
  700    A07 = AZ
         ...
         E207 = E2Z
         E307 = E3Z
         LON007 = .017453292519943E0 * DATA(5)
         LAT007 = .017453292519943E0 * DATA(6)
         X007 = DATA(7)
         Y007 = DATA(8)
         ML007 = MLFNSP (E007,E107,E207,E307,LAT007)
C
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON007,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT007,SGNA(2),DEGS(2),MINS(2),SECS(2))
...
C
  ...
  840       FORMAT (/' ERROR PJ08SP'/
     .               ' EQUAL LATITUDES FOR ST. PARALLELS ON OPPOSITE',
     .               ' SIDES OF EQUATOR')
            IERR = 081
            RETURN
         END IF
         LON008 = .017453292519943E0 * DATA(5)
         LAT0 = .017453292519943E0 * DATA(6)
         X008 = DATA(7)
         Y008 = DATA(8)
         SINPHI = SIN (LAT1)
         COSPHI = COS (LAT1)
         MS1 = MSFNSP (E,SINPHI,COSPHI)
         ML1 = MLFNSP (E008,E108,E208,E308,LAT1)
         IND = 0
         IF (DATA(9) .NE. ZERO) THEN
            IND = 1
            SINPHI = SIN (LAT2)
            COSPHI = COS (LAT2)
            MS2 = MSFNSP (E,SINPHI,COSPHI)
            ML2 = MLFNSP (E008,E108,E208,E308,LAT2)
            IF (ABS(LAT1-LAT2) .GE. EPSLN) THEN
         ...
         END IF
         GL = ML1 + MS1 / NS08
         ML0 = MLFNSP (E008,E108,E208,E308,LAT0)
         RH008 = A08 * (GL - ML0)
C
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LAT1,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT2,SGNA(2),DEGS(2),MINS(2),SECS(2))
         CALL RADDSP (LON008,SGNA(3),DEGS(3),MINS(3),SECS(3))
         CALL RADDSP (LAT0,SGNA(4),DEGS(4),MINS(4),SECS(4))
         IF (IND .NE. 0) THEN
...
C
  900    A09 = AZ
         ...
         E309 = E3Z
         KS009 = DATA(3)
         LON009 = .017453292519943E0 * DATA(5)
         LAT009 = .017453292519943E0 * DATA(6)
         X009 = DATA(7)
         Y009 = DATA(8)
         ML009 = A09 * MLFNSP (E009,E109,E209,E309,LAT009)
         IND09 = 1
         ESP = ES09
...
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON009,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT009,SGNA(2),DEGS(2),MINS(2),SECS(2))
...
C
 1000    A10 = AZZ
         LON010 = .017453292519943E0 * DATA(5)
         LAT010 = .017453292519943E0 * DATA(6)
         X010 = DATA(7)
         Y010 = DATA(8)
         SINP10 = SIN (LAT010)
         COSP10 = COS (LAT010)
...
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON010,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT010,SGNA(2),DEGS(2),MINS(2),SECS(2))
...
C
 1100    A11 = AZZ
         LON011 = .017453292519943E0 * DATA(5)
         LAT011 = .017453292519943E0 * DATA(6)
         X011 = DATA(7)
         Y011 = DATA(8)
         SINP11 = SIN (LAT011)
         COSP11 = COS (LAT011)
...
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON011,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT011,SGNA(2),DEGS(2),MINS(2),SECS(2))
...
C
 1200    A12 = AZZ
         LON012 = .017453292519943E0 * DATA(5)
         LAT012 = .017453292519943E0 * DATA(6)
         X012 = DATA(7)
         Y012 = DATA(8)
         SINP12 = SIN (LAT012)
         COSP12 = COS (LAT012)
...
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON012,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT012,SGNA(2),DEGS(2),MINS(2),SECS(2))
...
C
 1300    A13 = AZZ
         LON013 = .017453292519943E0 * DATA(5)
         LAT013 = .017453292519943E0 * DATA(6)
         X013 = DATA(7)
         Y013 = DATA(8)
         SINP13 = SIN (LAT013)
         COSP13 = COS (LAT013)
...
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON013,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT013,SGNA(2),DEGS(2),MINS(2),SECS(2))
...
C
 1400    A14 = AZZ
         LON014 = .017453292519943E0 * DATA(5)
         LAT014 = .017453292519943E0 * DATA(6)
         X014 = DATA(7)
         Y014 = DATA(8)
         SINP14 = SIN (LAT014)
         COSP14 = COS (LAT014)
...
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON014,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT014,SGNA(2),DEGS(2),MINS(2),SECS(2))
...
C
 1500    A15 = AZZ
         P = ONE + DATA(3) / A15
         LON015 = .017453292519943E0 * DATA(5)
         LAT015 = .017453292519943E0 * DATA(6)
         X015 = DATA(7)
         Y015 = DATA(8)
         SINP15 = SIN (LAT015)
         COSP15 = COS (LAT015)
...
C     LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON015,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT015,SGNA(2),DEGS(2),MINS(2),SECS(2))
...
C
 1600    A16 = AZZ
         LON016 = .017453292519943E0 * DATA(5)
         X016 = DATA(7)
         Y016 = DATA(8)
C
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON016,SGNA(1),DEGS(1),MINS(1),SECS(1))
...
C
 1700    A17 = AZZ
         LAT1 = .017453292519943E0 * DATA(6)
         LON017 = .017453292519943E0 * DATA(5)
         X017 = DATA(7)
         Y017 = DATA(8)
C
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LAT1,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LON017,SGNA(2),DEGS(2),MINS(2),SECS(2))
...
C
 1800    A18 = AZZ
         LON018 = .017453292519943E0 * DATA(5)
         X018 = DATA(7)
         Y018 = DATA(8)
C
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON018,SGNA(1),DEGS(1),MINS(1),SECS(1))
...
C
 1900    A19 = AZZ
         LON019 = .017453292519943E0 * DATA(5)
         X019 = DATA(7)
         Y019 = DATA(8)
C
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON019,SGNA(1),DEGS(1),MINS(1),SECS(1))
...
C
 2000    MODE = 0
         ...
         AL = A * BL * KS0 * COM / CON
         IF (ABS(LAT0).LT.EPSLN) TS0 = 1.0E0
         IF (ABS(LAT0).LT.EPSLN) D=1.0E0
         IF (ABS(LAT0).LT.EPSLN) EL=1.0E0
         IF (ABS(LAT0).GE.EPSLN) THEN
            TS0 = TSFNSP (E20,LAT0,SINPH0)
            CON = SQRT (CON)
            D = BL * COM / (COSPH0 * CON)
            F = D + SIGN (SQRT (MAX ((D * D - ONE), 0.0E0)) , LAT0)
            EL = F * TS0 ** BL
         END IF
 ...
 2050    FORMAT (/' INITIALIZATION PARAMETERS (OBLIQUE MERCATOR ''HOTI',
     ...
     .            ' FALSE NORTHING               =',F12.2,' METERS')
         IF (MODE .NE. 0) THEN
            ALPHA = .017453292519943E0 * DATA(4)
            LONC = .017453292519943E0 * DATA(5)
            G = HALF * (F - ONE / F)
            GAMMA = ASINSP (SIN (ALPHA) / D)
            LON020 = LONC - ASINSP (G * TAN (GAMMA)) / BL
C
C LIST INITIALIZATION PARAMETERS (CASE B).
C
            CALL RADDSP (ALPHA,SGNA(1),DEGS(1),MINS(1),SECS(1))
            CALL RADDSP (LONC,SGNA(2),DEGS(2),MINS(2),SECS(2))
            CALL RADDSP (LAT0,SGNA(3),DEGS(3),MINS(3),SECS(3))
...
C
 ...
 2040          FORMAT (/' ERROR PJ20SP'/
     .                  ' INPUT DATA ERROR')
               IERR = 201
               RETURN
            END IF
         END IF
         LON1 = .017453292519943E0 * DATA(9)
         LAT1 = .017453292519943E0 * DATA(10)
         LON2 = .017453292519943E0 * DATA(11)
         LAT2 = .017453292519943E0 * DATA(12)
         SINPHI = SIN (LAT1)
         TS1 = TSFNSP (E20,LAT1,SINPHI)
         SINPHI = SIN (LAT2)
         TS2 = TSFNSP (E20,LAT2,SINPHI)
         H = TS1 ** BL
         L = TS2 ** BL
         F = EL / H
         G = HALF * (F - ONE / F)
         ...
         P = (L - H) / (L + H)
         CALL RADDSP (LON2,SGNA(3),DEGS(3),MINS(3),SECS(3))
         DLON = LON1 - LON2
         IF (DLON .LT. -PI) LON2 = LON2 - 2.E0 * PI
         IF (DLON .GT.  PI) LON2 = LON2 + 2.E0 * PI
         DLON = LON1 - LON2
         LON020 = HALF * (LON1 + LON2) - ATAN (J * TAN (HALF * BL *
     .          DLON) / P) / BL
         DLON = ADJLSP (LON1 - LON020)
         GAMMA = ATAN (SIN (BL * DLON) / G)
         ALPHA = ASINSP (D * SIN (GAMMA))
         CALL RADDSP (LON1,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT1,SGNA(2),DEGS(2),MINS(2),SECS(2))
         CALL RADDSP (LAT2,SGNA(4),DEGS(4),MINS(4),SECS(4))
         CALL RADDSP (LAT0,SGNA(5),DEGS(5),MINS(5),SECS(5))
...
C
 2100    A21 = AZZ
         LON021 = .017453292519943E0 * DATA(5)
         X021 = DATA(7)
         Y021 = DATA(8)
         PR(1)=-0.062E0
         XLR(1)=0.9986E0
         ...
         XLR(20)=0.5322E0
         DO 2110 I=1,20
 2110    XLR(I)=XLR(I) * 0.9858E0
C
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON021,SGNA(1),DEGS(1),MINS(1),SECS(1))
...
C
         LAM=0
         CALL SERASP (FB,FA2,FA4,FC1,FC3,LAM)
         SUMA2=FA2
         SUMA4=FA4
         SUMB=FB
         SUMC1=FC1
         ...
         LAM=I
         CALL SERASP (FB,FA2,FA4,FC1,FC3,LAM)
         SUMA2=SUMA2+4.0E0*FA2
         SUMA4=SUMA4+4.0E0*FA4
         SUMB=SUMB+4.0E0*FB
         SUMC1=SUMC1+4.0E0*FC1
         SUMC3=SUMC3+4.0E0*FC3
 2210    CONTINUE
         DO 2220 I=18,72,18
         LAM=I
         CALL SERASP (FB,FA2,FA4,FC1,FC3,LAM)
...
C LIST RESULTS OF PARAMETER INITIALIZATION.
C
         CALL RADDSP (LON023,SGNA(1),DEGS(1),MINS(1),SECS(1))
         CALL RADDSP (LAT023,SGNA(2),DEGS(2),MINS(2),SECS(2))

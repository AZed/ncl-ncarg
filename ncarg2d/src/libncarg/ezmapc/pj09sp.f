C
C $Id: pj09sp.f,v 1.5 2008/07/27 00:17:10 haley Exp $
C
C                Copyright (C)  2000
...
C
      SUBROUTINE PJ09SP (COORD,CRDIO,INDIC)
C
...
C **** PARAMETERS ** A,E,ES,KS0,LON0,LAT0,X0,Y0,E0,E1,E2,E3,ESP,ML0,IND
      ...
      SAVE   /PRINZ0/
      COMMON /PC09SP/ A,LON0,X0,Y0,ES,ESP,E0,E1,E2,E3,KS0,LAT0,ML0,IND
      DATA ZERO,HALF,ONE,TWO,THREE /0.0E0,0.5E0,1.0E0,2.0E0,3.0E0/
      DATA FOUR,FIVE,SIX,EIGHT,NINE /4.0E0,5.0E0,6.0E0,8.0E0,9.0E0/
      DATA HALFPI /1.5707963267948966E0/
      DATA TEN /10.0E0/
      DATA EPSLN,NIT /1.0E-10,6/
C
C -- F O R W A R D   . . .
C
      IF (INDIC .EQ. 0) THEN
...
C
  240    SINPHI = SIN (LAT)
         ...
         ML = A * MLFNSP (E0,E1,E2,E3,LAT)
         PROJ(1) = KS0 * N * AL * (ONE + ALS / SIX * (ONE - T + C +
     .             ALS / 20.0E0 * (FIVE - 18.0E0 * T + T * T + 72.0E0 *
     .             C - 58.0E0 * ESP))) + X0
         PROJ(2) = KS0 *(ML - ML0 + N * TQ *(ALS *(HALF + ALS / 24.0E0 *
     .             (FIVE - T + NINE * C + FOUR * C * C + ALS / 30.0E0 *
     .             (61.0E0 - 58.0E0 * T + T * T + 600.0E0 * C -
     .             330.0E0 * ESP))))) + Y0
         CRDIO(1) = PROJ(1)
         CRDIO(2) = PROJ(2)
         RETURN
...
C
         ...
         CRDIO(2) = GEOG(2)
         RETURN
  330    GEOG(1) = ADJLSP (ATAN2 (G,H) + LON0)
         CRDIO(1) = GEOG(1)
         CRDIO(2) = GEOG(2)
         RETURN
C
 ...
 2030    FORMAT (/' ERROR PJ09SP' /
  ...
  400    SINPHI = SIN (PHI)
         ...
         D = X / (N * KS0)
         DS = D * D
         GEOG(2) = PHI - (N * TANPHI * DS / R) * (HALF - DS / 24.0E0 *
     .             (FIVE + THREE * T + TEN * C - FOUR * CS - NINE * ESP
     .             - DS / 30.0E0 * (61.0E0 + 90.0E0 * T + 298.0E0 * C +
     .             45.0E0 * TS - 252.0E0 * ESP - THREE * CS)))
         GEOG(1) = ADJLSP (LON0 + (D * (ONE - DS / SIX * (ONE + TWO *
     .             T + C - DS / 20.0E0 * (FIVE - TWO * C + 28.0E0 * T -
     .             THREE * CS + EIGHT * ESP + 24.0E0 * TS))) / COSPHI))
         CRDIO(1) = GEOG(1)
         CRDIO(2) = GEOG(2)
         RETURN
      END IF

C
C $Id: pj17sp.f,v 1.5 2008/07/27 00:17:11 haley Exp $
C
C                Copyright (C)  2000
...
C
      SUBROUTINE PJ17SP (COORD,CRDIO,INDIC)
C
...
C **** PARAMETERS **** A,LON0,X0,Y0,LAT1 *******************************
      ...
      SAVE   /PRINZ0/
      COMMON /PC17SP/ A,LON0,X0,Y0,LAT1
...
C
 ...
 2020    FORMAT (/' ERROR PJ17SP'/
     .            ' INPUT DATA ERROR')
         IERR = 174
         RETURN
  240    GEOG(1) = ADJLSP (LON0 + X / (A * COS(LAT1) ))
         CRDIO(1) = GEOG(1)
         CRDIO(2) = GEOG(2)
         RETURN
      END IF

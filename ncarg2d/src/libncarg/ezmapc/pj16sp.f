C
C $Id: pj16sp.f,v 1.5 2008/07/27 00:17:11 haley Exp $
C
C                Copyright (C)  2000
...
C
      SUBROUTINE PJ16SP (COORD,CRDIO,INDIC)
C
...
C
 ...
 2020    FORMAT (/' ERROR PJ16SP'/
  ...
  230    CON = ABS (GEOG(2)) - HALFPI
         ...
         CRDIO(2) = GEOG(2)
         RETURN
  240    GEOG(1) = ADJLSP (LON0 + X / (A * COS (GEOG(2))))
         CRDIO(1) = GEOG(1)
         CRDIO(2) = GEOG(2)
         RETURN
      END IF

C
C	$Id: pwrzt.f,v 1.5 2008/07/27 00:17:23 haley Exp $
C                                                                      
C                Copyright (C)  2000
...
C
C
C USAGE                  CALL PWRZT (X,Y,Z,ID,N,ISIZE,LIN3,ITOP,ICNT)
...
C
  ...
  109 XC = XC-CT*2.
      ...
      XC = XC-CT*FNNM1*6.
      YC = YC-ST*FNNM1*6.
  110 CALL INITZT (INT(XC),INT(YC),1,IDUM1,IDUM2,2)
      CALL INITZT (INT(XC+CT*6.*FNNM1),INT(YC+ST*6.*FNNM1),2,IDUM1,
     +             IDUM2,2)
      CALL INITZT (INT(XC),INT(YC),IDUM1,IDUM2,IDUM3,3)
      DO 114 K=1,NN
         XB = XC
         YB = YC
         IP = 1
...
C
      ...
      CALL INITZT (INT(XC+.5),INT(YC+.5),IDUM1,IDUM2,IDUM3,3)
          IP = 1
          GO TO 113
  112 CALL INITZT (INT(XC+.5),INT(YC+.5),IDUM1,IDUM2,IDUM3,4)
  113  CONTINUE
  114 CONTINUE
C

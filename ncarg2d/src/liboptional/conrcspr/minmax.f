C
C	$Id: minmax.f,v 1.5 2008/07/27 00:23:02 haley Exp $
C                                                                      
C                Copyright (C)  2000
...
C
         ...
         MY = IFY - SIZEM
         TMY = CPUY (MY)
         CALL PWRTM (FX(X,Y),TMY,IA(1:NC),ISIZEM,0,0)
  112    IM = 1
...
C
C ****************************** ENTRY PNTVAL **************************
C     ENTRY PNTVAL (Z,L,MM,NN,SSIZEM,AASH,JOFFDT)
C
  128 CONTINUE
      II = (M-1+24)/24
      JJ = (N-1+48)/48
      ...
      DO 130 J=NJQ,N,JJ
         ...
         DO 129 I=NIQ,M,II
            ...
            CALL PWRTM (FX(X,Y),FY(X,Y),IA(1:NC),ISIZEM,0,0)
  129    CONTINUE
  130 CONTINUE
      RETURN
      END

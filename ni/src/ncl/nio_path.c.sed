/*
 *      $Id: nio_path.c.sed,v 1.2 2008/07/27 12:20:39 haley Exp $
 */
/************************************************************************
*                                                                       *
*                            Copyright (C)  2000                        *
...
/*
 * This is the NIO version of ncarg_path.h. It is included here to
	...
	const char	*name;
{
	...
	if(!found){
		NhlPError(NhlFATAL,E_UNKNOWN, "Unknown environment specifier (%s)", name);
		return(NULL);
	}
	...
	if(!env_name){
		NhlPError(NhlFATAL,E_UNKNOWN,"Unable to create Env Name?? for (%s)",name);
		return NULL;
	}
